...
const SportPathsUpdate = ({ dataAR, dataEN }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (file === null) {
      await fetch(`/api/sport/paths/update/${id}`, {
        method: "PUT",
        headers: {
...
export async function getServerSideProps(context: any) {
    const dataEN = await (await fetch(`http://127.0.0.1:3000/api/sport/paths/${context.params.id}?language=en`)).json();
    const dataAR = await (await fetch(`http://127.0.0.1:3000/api/sport/paths/${context.params.id}?language=ar`)).json();
...
export default SportPathsUpdate