...
const updateTouristicItem = (
    ...
    try {
        conn.query(`UPDATE touristicen SET title='${english.titleEN}', description='${english.descriptionEN}', link_to_read_more='${english.linkToReadMore}', map_source='${english.mapSource}', map_link='${english.mapLink}', location='${english.locationEN}', category='${english.categoryEN}', status=${english.status}  WHERE id=${id}`, (err) => {
            if (err) throw new Error("There is an error to update your data!");
        });

        conn.query(`UPDATE touristicar SET title='${arabic.titleAR}', description='${arabic.descriptionAR}', link_to_read_more='${english.linkToReadMore}', map_source='${english.mapSource}', map_link='${english.mapLink}', location='${arabic.locationAR}', category='${arabic.categoryAR}', status=${english.status}  WHERE id=${id}`, (err) => {
            if (err) throw new Error("There is an error to update your data!");
        });

        if (english.image !== '') {
            conn.query(`UPDATE touristicen SET image='${english.image}'`, (err) => {
                if (err) throw new Error("There is an error to update your data!");
            });

            conn.query(`UPDATE touristicar SET image='${english.image}'`, (err) => {
                if (err) throw new Error("There is an error to update your data!");
            });
        }
...
export default updateTouristicItem;