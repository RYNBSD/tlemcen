...
const deleteTouristicItem = (
    ...
    try {
        conn.query(`DELETE FROM touristicar WHERE id=${parseInt(id as string)}`, (err) => {
            if (err) throw new Error("Can\'t delete the item");
        });

        conn.query(`DELETE FROM touristicen WHERE id=${parseInt(id as string)}`, (err) => {
            if (err) throw new Error("Can\'t delete the item");
        });
...
export default deleteTouristicItem;