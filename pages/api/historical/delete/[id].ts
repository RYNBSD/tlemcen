...
const deleteHistoricalItem = (
    ...
    try {
        conn.query(`DELETE FROM historicalar WHERE id=${parseInt(id as string)}`, (err) => {
            if (err) throw new Error("Can\'t delete the item");
        });

        conn.query(`DELETE FROM historicalen WHERE id=${parseInt(id as string)}`, (err) => {
            if (err) throw new Error("Can\'t delete the item");
        });
...
export default deleteHistoricalItem;