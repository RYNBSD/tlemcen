...
const updateSportPathsItem = (
    ...
    try {
        conn.query(`UPDATE sportpathsen SET title='${english.titleEN}', description='${english.descriptionEN}', link_to_read_more='${english.linkToReadMore}', map_source='${english.mapSource}', map_link='${english.mapLink}', location='${english.locationEN}', category='${english.categoryEN}', status=${english.status}  WHERE id=${id}`, (err: any) => {
            if (err) throw new Error("There is an error to update your data!");
        });

        conn.query(`UPDATE sportpathsar SET title='${arabic.titleAR}', description='${arabic.descriptionAR}', link_to_read_more='${english.linkToReadMore}', map_source='${english.mapSource}', map_link='${english.mapLink}', location='${arabic.locationAR}', category='${arabic.categoryAR}', status=${english.status}  WHERE id=${id}`, (err: any) => {
            if (err) throw new Error("There is an error to update your data!");
        });

        if (english.image !== '') {
            conn.query(`UPDATE sportpathsen SET image='${english.image}'`, (err: any) => {
                if (err) throw new Error("There is an error to update your data!");
            });

            conn.query(`UPDATE sportpathsar SET image='${english.image}'`, (err: any) => {
                if (err) throw new Error("There is an error to update your data!");
            });
        }
...
export default updateSportPathsItem;