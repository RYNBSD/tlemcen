...
const newItem = async (
    ...
    try {
        ...
        const insertEnglishSportPaths = `
            INSERT INTO sportpathsen (title, description, link_to_read_more, image, map_source, map_link, location, category, status) VALUES ('${english.titleEN}', '${english.descriptionEN}', '${english.linkToReadMore}', '${english.image}', '${english.mapSource}', '${english.mapLink}', '${english.locationEN}', '${english.categoryEN}', ${english.status})
        `;

        const insertArabicSportPaths = `
            INSERT INTO sportpathsar (title, description, link_to_read_more, image, map_source, map_link, location, category, status) VALUES ('${arabic.titleAR}', '${arabic.descriptionAR}', '${english.linkToReadMore}', '${english.image}', '${english.mapSource}', '${english.mapLink}', '${arabic.locationAR}', '${arabic.categoryAR}', ${english.status})
        `;

        conn.query(insertEnglishSportPaths, (err: any) => {
            if (err) {
                throw new Error("Invalid to insert");
            }
        });

        conn.query(insertArabicSportPaths, (err: any) => {
            if (err) {
                throw new Error("Invalid to insert");
            }
        });
